...
export const saveCookies = (fileName: string, fileOptions?: Partial<FileOptions>): Action<InjectBrowserPage & {fileOptions: FileOptions}> =>
 ...
 */
export const loadCookies = (fileName: string, fileOptions?: Partial<FileOptions>): Action<InjectBrowserPage & {fileOptions: FileOptions}> =>
  async({page, fileOptions: injectedOptions}) => {
    // fileOptions is higher order param that overwrites injected options
    const hydratedOptions = enrichFileOptionsWithDefaults({...injectedOptions, ...fileOptions})
 ...
 * @param injects
 */
export const getCookies = (...urls: string[]): Action<InjectBrowserPage, Protocol.Network.Cookie[]> =>
  async({page}) =>
    page.cookies(...urls)
 ...
 * @param cookies
 */
export const deleteCookies = (...cookies: Protocol.Network.Cookie[]): Action<InjectBrowserPage & Partial<InjectValue>> =>
  async({page, value}) => {
    if (cookies.length === 0) {
