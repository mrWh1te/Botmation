 ...
 * @param url
 */
export const goTo = (url: string, goToOptions?: Partial<WaitForOptions>): Action<injectsPage> =>
  async({page}) => {
    if (page.url() === url) {
      return
    }
 ...
 * @param options
 */
export const goBack = (options?: WaitForOptions): Action<injectsPage> =>
  async({page}) => {
 ...
 * @param options
 */
export const goForward = (options?: WaitForOptions): Action<injectsPage> =>
  async({page}) => {
 ...
 * @param options
 */
export const reload = (options?: WaitForOptions): Action<injectsPage> =>
  async({page}) => {
 ...
 */
export const waitForNavigation: Action<injectsPage> = async({page}) => {
 ...
 */
export const scrollTo = (htmlSelector: string, waitTimeForScroll: number = 2500): Action<injectsPage> =>
  chain<injectsPage>(
