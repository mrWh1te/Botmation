 ...
 */
export const abort = (assembledLines = 1, pipeValue?: PipeValue): Action<AbortLineSignal> =>
 ...
 */
export const abortPipe = (value: CaseValue, abortPipeValue: PipeValue = undefined, assembledLines: number = 1): Action<AbortLineSignal|PipeValue|CasesSignal> =>
  pipeCase(value)(
  ...
  async(injects) => {
    ...
    do {
      ...
      for(const action of actions) {
        ...
        actionResult = await action({...injects, value: actionResult})

        // unique restart aborting behavior
        if (isAbortLineSignal(actionResult)) {
          // cannot restart an infinity abort, if desirable, create another Action - maybe with a customizing HO function
