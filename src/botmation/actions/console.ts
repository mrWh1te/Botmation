...
export const log = (message?: string): BotAction5 => async (page, ...injects: any[]) => {
  if (message) {
    logMessage(message)
  }
...
export const warning = (warning?: string): BotAction5 => async (page, ...injects: any[]) => {
  if (warning) {
    logWarning(warning)
  }
...
export const error = (error?: string): BotAction5 => async (page, ...injects: any[]) => {
  if (error) {
    logError(error)
  }
